...
namespace Aspose.Barcode.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeBarCodeContext.atcc = new AsposeBarCodeContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeBarCodeContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeBarCodeContext.Current.Cache.Add(
				   SessionID,
					resources,
