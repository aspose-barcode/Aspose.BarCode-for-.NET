...
function loadDocument(callback) {
	...
	$.ajax({
		type: 'POST',
		url: getApplicationPath('ViewerInfo'),
		data: JSON.stringify(data),
		global: false,
		contentType: "application/json",
		success: function (returnedData) {
			...
			if (returnedData.info.navigationPane && returnedData.info.navigationPane.length > 0) {
				const STYLES = {
					1  : 'header-1',
					2  : 'header-2',
					3  : 'header-3',
				...
				navigationPane = returnedData.info.navigationPane;
			...
			documentData = returnedData.info;
...
function loadThumbnails() {
	...
	$.ajax({
		type: 'POST',
		url: getApplicationPath('ViewerThumbnails'),
		data: JSON.stringify(data),
		global: false,
		contentType: "application/json",
		success: function (returnedData) {
			...
			$.each(returnedData.thumbnails, function (index, elem) {
...
function appendHtmlContent(pageNumber, documentName, callback) {
	...
	if (!gd_page.hasClass('loaded')) {
		...
		$.ajax({
			type: 'POST',
			url: getApplicationPath('ViewerSlide'),
			data: JSON.stringify(data),
			global: false,
			contentType: "application/json",
			success: function (htmlData) {
				...
				renderPage(gd_page, htmlData.slide, documentName);
			},
			error: function (xhr, status, error) {
				fadeAll(false);
				var err = eval("(" + xhr.responseText + ")");
		...
		init: function (options) {
			...
			$('#gd-pages').on('click', 'a', function(el) {
				...
				if (h.startsWith('#_page')) {
					const pageNumber = parseInt(h.split('_')[1].substring(4));
					appendHtmlContent(pageNumber, options.defaultDocument, function() {
						window.location.href = h;
						setNavigationPageValues(pageNumber, totalPageNumber);
						if (pageNumber > 1) { // load previous page
							appendHtmlContent(pageNumber - 1, options.defaultDocument, function() { // load next page
								if (pageNumber < totalPageNumber) {
									appendHtmlContent(pageNumber + 1, options.defaultDocument, function() {});
								}
							});
						}
					});
